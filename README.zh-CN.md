...
可复用的引擎代码；业务 query、品牌数据、raw runs，以及可选的 DuckDB/dashboard 产物都应
...
- **可选分析扩展**：DuckDB、跨运行聚合和静态 HTML 均为 opt-in，不进入核心报告路径。
...
flowchart LR
    ...
    CSV -. "optional" .-> DuckDB
    DuckDB -.-> Dashboard["可选 static dashboard"]
...
仅在需要临时、自包含 HTML 报告时添加 `--html-report`。DuckDB 与独立静态 dashboard
是下文描述的可选扩展，默认流程不会创建它们。
...
Dashboard 是次级静态 HTML 导出，不需要后端服务、登录、React/Vue 应用或远程数据库，
且刻意不属于默认报告流程。
...
src/geo_monitor/
  ...
  analysis/              # orchestration、稳定 contract、history adapter
  ...
  db.py                  # 可选 DuckDB analysis cache
  dashboard.py           # 可选 legacy static dashboard
